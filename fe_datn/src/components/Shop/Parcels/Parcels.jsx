...
const Parcels = ({status,nextStatus}) => {
  const { currentUser } = useContext(AuthContext);

  const { isLoading, error, data } = useQuery(["parcels",currentUser?.user?.id], () =>
    makeRequest.get("/get-parcels-by-status", {
  ...
  return (
    <>
    <div className="cart-ctable mb-[20px]">
