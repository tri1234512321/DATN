...
const Friends = () => {
  ...
  //   (user) => {
  //     return makeRequest.post("social/friends", { userId:currentUser.id,  friendUserId:user.id});
  //   },
  //   {
  //     onSuccess: () => {
  ...
  const deleteMutation = useMutation(
    (user) => {
      return makeRequest.delete("/social/friends/" + user.friendId);
    },
    {
      onSuccess: () => {
        // Invalidate and refetch
        ...
        : data.map((friend,index) => (
            <div className="friend" key={100+index}>
              ...
              <div className="px-[8px] py-[5px]">
                <p className="text-slate-950 font-[600] py-[8px]">{friend.firstName}</p>
