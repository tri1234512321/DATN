  ...
  return async (dispatch, getState) => {
    try {
      setTimeout(async () => {
        ...
        let res = await shopService.createNewItemProduct(data);
        if (res && res.EC === 0) {
...
export const fetchAllItemProductStart = (access_token) => {
  return async (dispatch, getState) => {
    try {
      ...
      const res = await shopService.shopGetAllProduct("All", access_token);
      if (res && res.EC === 0) {
...
export const deleteProductStart = (id, access_token) => {
  return async (dispatch, getState) => {
    try {
      const res = await shopService.deleteProductService(id, access_token);
...
export const updateProductStart = (dataItem) => {
  return async (dispatch, getState) => {
    try {
      const res = await shopService.updateProductService(dataItem);
